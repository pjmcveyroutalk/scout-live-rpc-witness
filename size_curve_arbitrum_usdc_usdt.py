#!/usr/bin/env python3
"""Scout Arbitrum USDC->USDt0 size-curve witness. Read-only GET quotes only."""
...
HEADERS=[("Host",HOST),("User-Agent","Scout-Arbitrum-Size-Curve/1.0.0"),
...
def one_quote(usdc_units:int):
    ...
    try:
        ...
        conn.endheaders(); resp=conn.getresponse(); raw=resp.read(); status=resp.status
    finally:
        conn.close()
    obj=json.loads(raw.decode())
    r=obj.get("priceRoute") if isinstance(obj,dict) else None
    accepted=(status==200 and isinstance(r,dict)
              and str(r.get("srcToken","")).lower()==USDC.lower()
              and str(r.get("destToken","")).lower()==USDT0.lower()
              and str(r.get("srcAmount"))==str(amount)
              and str(r.get("destAmount","")).isdigit() and int(r.get("destAmount",0))>0
              and r.get("side")=="SELL" and str(r.get("network"))==str(CHAIN_ID)
              and isinstance(r.get("bestRoute"),list) and len(r["bestRoute"])>0)
    return {
        "accepted":accepted,"input_units_usdc":usdc_units,"input_amount_base_units":amount,
        "gross_output_amount_base_units":int(r["destAmount"]) if accepted else None,
        "gross_output_units_usdt":int(r["destAmount"])/1_000_000 if accepted else None,
        "gas_cost":r.get("gasCost") if isinstance(r,dict) else None,
        "gas_cost_usd":r.get("gasCostUSD") if isinstance(r,dict) else None,
        "src_usd":r.get("srcUSD") if isinstance(r,dict) else None,
        "dest_usd":r.get("destUSD") if isinstance(r,dict) else None,
...
def main():
    ...
    started=now(); d=root/("size-curve-arbitrum-"+started.replace(":","").replace("-","").replace(".","_")); d.mkdir()
    ...
    ev={"schema":"scout.size_curve.arbitrum.v1","accepted":accepted,"observation_only":True,"execution_authority":False,
        "chain":"arbitrum-one","provider":"velora-paraswap","pair":"USDC/USDT","output_representation":"USD₮0",
        "sizes_usdc":list(SIZES_USDC),"dns":{"hostname":HOST,"addresses":addresses},
    ...
    print(json.dumps({"accepted":accepted,"gross_outputs":[x["gross_output_units_usdt"] for x in observations],
                      "gas_cost_usd":[x["gas_cost_usd"] for x in observations],"execution_authority":False},indent=2))
