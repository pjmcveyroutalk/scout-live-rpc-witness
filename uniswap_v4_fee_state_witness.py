#!/usr/bin/env python3
"""Scout T661 Uniswap v4 protocol-fee-state witness for Arbitrum One.

Read-only only:
- hard-coded Arbitrum public RPC
- hard-coded canonical Uniswap v4 StateView
- hard-coded USDC/USD₮0 pool id from Scout's accepted Velora replay
- eth_call only, using getSlot0(bytes32)

Captures exact request/response bytes and SHA-256 evidence.
...
def make_request(body: bytes) -> Tuple[Optional[int], Dict[str, str], bytes, Optional[str]]:
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": f"Scout-T661-V4-Fee-State/{WITNESS_VERSION}",
        },
    )
...
def decode_slot0_result(raw_hex: str) -> Dict[str, int]:
    ...
    sqrt_price_x96 = words[0] & ((1 << 160) - 1)
    tick = decode_int24(words[1])
    protocol_fee = words[2] & 0xFFFFFF
    lp_fee = words[3] & 0xFFFFFF
...
def main() -> int:
    ...
    checks: Dict[str, bool] = {
        ...
        "lp_fee_matches_captured_route": False,
    ...
    effective_usdc_to_usdt0_pips: Optional[int] = None
    ...
    if isinstance(obj, dict):
        ...
        if checks["result_present"] and checks["rpc_error_absent"]:
            try:
                ...
                checks["lp_fee_matches_captured_route"] = (
                    decoded["lpFee"] == EXPECTED_STATIC_LP_FEE_PIPS
                )

                effective_usdc_to_usdt0_pips = effective_swap_fee_pips(
    ...
    evidence = {
        "schema": "scout.t661.uniswap_v4_protocol_fee_state.v1",
        "accepted": accepted,
        ...
        "effective_usdc_to_usdt0_fee_pips": effective_usdc_to_usdt0_pips,
        "effective_usdc_to_usdt0_fee_bps": (
            effective_usdc_to_usdt0_pips / 100.0
            if effective_usdc_to_usdt0_pips is not None
            else None
        ),
        "validation": checks,
